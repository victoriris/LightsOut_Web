...
$(() => {
    ...
    $('#gameInput').submit(function (event) {
        ...
        var lightsNumber = Math.trunc((rows * cols) * (percentToTurnOn / 100));
        for (var i = 0; i < lightsNumber; i++) {
            var randomRow = getRandomNumber(rows);
            var randomColumn = getRandomNumber(cols);
